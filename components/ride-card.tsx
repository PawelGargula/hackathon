    ...
    <div className="group flex flex-col gap-3 rounded-2xl bg-card p-4 ring-1 ring-border transition-shadow hover:shadow-md">
      ...
      <div className="flex items-center justify-between gap-2 border-t border-border pt-3">
        {ride.kind === "CAR" && ride.driver ? (
          <DriverChip driver={ride.driver} />
        ) : (
          <span className="text-xs text-muted-foreground">Kurs publiczny MPK</span>
        )}
        <Link
          href={target}
    ...
    <div className={cn("flex items-center gap-2", className)}>
      ...
      <div className="leading-tight">
        <p className="flex items-center gap-1 text-sm font-medium">
          ...
          {driver.verified && (
            <BadgeCheck className="size-3.5 text-primary" aria-label="zweryfikowany" />
          )}
        </p>
        {driver.rating != null && (
